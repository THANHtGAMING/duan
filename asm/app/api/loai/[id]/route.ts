...
export async function DELETE(req: Request, { params }: { params: { id: string } }) {
    ...
    if (!loai) return NextResponse.json({ thong_bao: "Khong tim thay" }, { status: 404 })
    await loai.destroy()
    return NextResponse.redirect(new URL("/admin/loai", req.url))
}
...
export async function POST(req: Request, { params }: { params: { id: string } }) {
  ...
  const loai = await LoaiModel.findByPk(params.id);
  if(!loai) return  NextResponse.json({thong_bao: 'Khong tim thay loai'}, {status:404})
